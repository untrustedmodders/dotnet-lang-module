...
public static class NativeInterop
{
    ...
    private static ManagedClass InitManagedClass(Guid assemblyGuid, nint classHolderPtr, Type type)
    {
        ...
        List<nint> baseClassNamesPtrs = [];
        Type? currentType = type.BaseType;

        while (currentType != null)
        {
	        baseClassNamesPtrs.Add(Marshal.StringToHGlobalAnsi(currentType.Name));
	        currentType = currentType.BaseType;
        }

        ManagedClass_Create(ref assemblyGuid, classHolderPtr, type.GetHashCode(), typeNamePtr, (uint)baseClassNamesPtrs.Count, baseClassNamesPtrs.ToArray(), out ManagedClass managedClass);

        foreach (var baseClassNamePtr in baseClassNamesPtrs)
        {
	        Marshal.FreeHGlobal(baseClassNamePtr);
        }
    ...
    private static unsafe void HandleParameters(nint paramsPtr, MethodInfo methodInfo, out object?[]? parameters)
    {
        ...
        for (int i = 0; i < numParams; i++)
        {
            ...
            if (paramType.IsEnum)
            {
                ...
                parameters[i] = Enum.ToObject(paramType, Extract(underlyingType, paramAttributes, paramPtr));
            } 
            else
            {
                parameters[i] = Extract(paramType, paramAttributes, paramPtr);
            }
        }
    }
    ...
    private static unsafe void HandleReferences(nint paramsPtr, MethodInfo methodInfo, object?[]? parameters)
    {
        ...
        for (int i = 0; i < numParams; i++)
        {
            ...
            if (paramValue == null)
            {
	            throw new InvalidOperationException("Reference cannot be null");
            }
    ...
    public static void InvokeMethod(Guid managedMethodGuid, Guid thisObjectGuid, nint paramsPtr, nint outPtr)
    {
        ...
        if (returnValue == null)
        {
	        throw new InvalidOperationException("Return cannot be null");
        }
    ...
    private static unsafe object Extract(Type paramType, object[] paramAttributes, nint paramPtr)
    {
	    if (paramType.IsByRef)
	    {
		    paramType = TypeUtils.ConvertToUnrefType(paramType);
    ...
    private static Func<object[], object> InternalInvoke(Delegate d)
    {
	    ...
	    return parameters =>
	    {
		    ...
		    {
			    object paramValue = parameters[i];
			    ManagedType paramType = parameterTypes[j];
			    ValueType valueType = paramType.ValueType;
			    ...
			    switch (valueType)
			    {
				    case ValueType.String:
				    {
					    args[j] = NativeMethods.GetStringData((nint)paramValue);
					    break;
				    }
				    case ValueType.ArrayBool:
				    {
		    ...
		    if (hasRefs)
		    {
			    for (int i = startIndex, j = 0; i < parameters.Length; i++, j++)
			    {
				    ...
				    if (paramType.IsByRef)
				    {
					    ...
					    {
						    case ValueType.String:
							    NativeMethods.AssignString((nint)paramValue, (string)args[j]);
							    break;
						    case ValueType.ArrayBool:
						    {
							    var arr = (bool[])args[j];
    ...
    internal static bool NeedMarshal(Type paramType)
    {
	    ...
	    {
		    var methodInfo = paramType.GetMethod("Invoke") ?? throw new InvalidOperationException("Invalid delegate!");
		    if (NeedMarshal(methodInfo.ReturnType) || methodInfo.GetParameters().Any(p => NeedMarshal(p.ParameterType)))
		    {
			    return true;
		    }
    ...
    private static extern void ManagedClass_Create(ref Guid assemblyGuid, nint classHolderPtr, int typeHash, nint typeNamePtr, uint numBaseClasses, nint[] baseClassNames, [Out] out ManagedClass result);
